def convert_and_extend(input_str, required_size):
        ...
        #     binary_str = bin(num)[2:]
        # else:
        #     binary_str = bin((2**(required_size+1)) + num)[3:]  # Add leading "1" for negative
        # if len(binary_str) < required_size:
        #     binary_str = '0' * (required_size - len(binary_str)) + binary_str
        # return binary_str[-required_size:]
...
def binary_string_output(string1):
    ...
    #     else:
    #         return("a")
    #     bin_str = "1100011" + rs1req + rs2req + func3 + rdreq + '00000'
    #     return bin_str
    elif opcode in B_instructions:
        list2=list1[1].split(",")
        reg2=list2[1]
...
pointer1 = open(r"C:\Users\atina\Downloads\CO Project evaluation framework\CO Project evaluation framework\automatedTesting\tests\assembly\simpleBin\test1.txt", "r")
...
pointer2 = open(desktop_path + "\\open21.txt", "w")
error=0
lengthofprogram=0
...
if lengthofprogram<=64:
    str1=list1[length-1]
    str1=str1.strip()
    if str1 != "beq zero,zero,0" and str1 != "beq zero,zero,00000000":
